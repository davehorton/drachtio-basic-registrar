...
function handler(log) {
  return async(req, res) => {
    ...
    const logger = log.child({'Call-ID': req.get('Call-ID')});
    ...
    const headers = { from };
    if (req.has('Authorization') || req.has('Proxy-Authorization')) {
      try {
        const obj = await registrar.queryTransaction(req.get('Call-ID'));
        Object.assign(headers, {
          'Call-ID': obj.callId,
          'CSeq': `${++obj.seq} INVITE`
        });
        logger.info(headers, 'headers on uac invite with creds');
      } catch (err) {
        logger.error(err, `Error retrieving transaction for ${req.get('Call-ID')}`);
      }
    }
    try {
      ...
      const {uas, uac} = await srf.createB2BUA(req, res, dest, {
        ...
        proxyRequestHeaders: ['to', 'supported', 'allow', 'content-type',
          'user-agent', 'authorization', 'proxy-authorization'],
        proxyResponseHeaders: ['accept', 'allow', 'allow-events', 'authenticate', 'proxy-authenticate', 'www-authenticate'],
      }, {
        cbRequest: (err, invite) => {
          if (!(req.has('Authorization') || req.has('Proxy-Authorization'))) {
            registrar.addTransaction(req.get('Call-ID'), {
              callId: invite.get('Call-Id'),
              seq: invite.getParsedHeader('CSeq').seq
            }, 3);
          }
        }
      });
