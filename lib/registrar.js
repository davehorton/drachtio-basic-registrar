...
function makeTransactionKey(key) {
  return `txn:${key}`;
}
...
class Registrar {
  ...
  async addTransaction(key, obj, expires) {
    this.logger.info(`Registrar#addTransaction ${key} with value ${JSON.stringify(obj)} and ${expires}`);
    const k = makeUserKey(key);
    try {
      const result = await this.client
        .multi()
        .hmset(k, obj)
        .expire(k, expires)
        .execAsync();
      this.logger.info(`Registrar#addTransaction ${key}: ${result}`);
    } catch (err) {
      this.logger.error(err, `Error adding transaction ${key}`);
    }
  }

  async queryTransaction(key) {
    const k = makeUserKey(key);
    const result = await this.client.hgetallAsync(k);
    this.logger.info(`Registrar#queryTransaction: ${key} returned ${JSON.stringify(result)}`);
    return result;
  }
}
